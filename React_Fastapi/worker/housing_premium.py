...
def deepgram_transcribe(audio_url):
    ...
    try:
        ...
        # hitting a flat 30s write timeout. ~1s per MB, floor of 60s.
...
def worker():
    ...
    while True:
        ...
        try:
            ...
            # Recording file lives in routing_numbers for this table
            transcription = deepgram_transcribe(row["routing_numbers"])

            if not transcription:
                # Transcription failed (download error, Deepgram error,
                # or timeout on a large file). Don't insert a blank
                # CallDetails row and don't mark flag=1 — that would
                # silently lose this call forever. Mark it flag=2
                # ("failed") instead so it's excluded from the flag=0
                # queue but still visible for manual reprocessing,
                # rather than looping forever on the same broken file.
                logging.error(
                    f"Transcription failed for Call ID {row['id']} "
                    f"({row['routing_numbers']}) - marking flag=2 for review"
                )
                dcur.execute(
                    "UPDATE CR_housing_premium SET flag=2 WHERE id=%s",
                    (row["id"],)
                )
                time.sleep(2)
                continue
            ...
            # leg_b_start_time carries the actual call timestamp;
            # call_date is date-only, used as a fallback
            call_datetime = row["leg_b_start_time"]
            ...
            insert_data = {
                ...
                "AgentName": row["call_group_2"],
                "MobileNo": row["call_center"],
            ...
            dcur.execute(
                "UPDATE CR_housing_premium SET flag=1 WHERE id=%s",
                (row["id"],)
            )
...
if __name__ == "__main__":
    logging.info("CR Housing Premium Worker Started")
    worker()