    ...
    try:
        ...
        for attempt in range(3):
            try:
                ...
                if audio_res.status_code == 403:
                    alt_url = (
                        audio_url.replace("/MP3/", "/")
                        .replace(".mp3", ".wav")
                    )

                    logging.warning(
                        f"[Attempt {attempt+1}] 403 -> WAV fallback: {alt_url}"
                    )
                ...
                if audio_res.status_code == 404:
                    ...
                    if match:
                        ...
                        proxy_url = (
                            f"http://192.168.11.251/"
                            f"{ip_underscore}/{date_part}/{filename}"
                        )

                        logging.warning(
                            f"[Attempt {attempt+1}] 404 -> Proxy: {proxy_url}"
                        )
                ...
                logging.warning(
                    f"[Attempt {attempt+1}] Failed ({audio_res.status_code})"
                )
            ...
            logging.error(f"Audio fetch failed: {audio_url}")
            return ""

        if not audio_res.content:
            logging.error("Downloaded audio is empty.")
            return ""
        ...
        logging.info(
            f"Whisper finished in {time.time()-start:.2f}s"
        )
        ...
        logging.exception(f"Whisper error: {e}")
        return ""
    ...
    while True:
        ...
        try:
            ...
            insert_data = {
                ...
                "CallDisposition": gpt_data.get("CallDisposition"),
